...
class CartPoleEnv_adv(gym.Env):
    ...
    def __init__(self):
        ...
        self.friction_coef = 1.
        self.joint_friction_coef = 0.1
        self.observe_noise_scale = np.array([0.05, 1., 0.02, 1.])
    ...
    def step(self, action, impulse=0, process_noise=np.zeros([5])):
        ...
        xacc = -self.friction_coef * x_dot / self.total_mass + temp - self.polemass_length * thetaacc * costheta / self.total_mass
        ...
        if abs(x) < 0.04 and abs(x_dot[0]) <0.1 and abs(theta) <0.05 and abs(theta_dot[0])<0.05:
            swing_up_success = True
            done = True
        else:
        ...
        cost = 1* x**2/self.x_threshold**2 + 20 *(theta/ self.theta_threshold_radians)**2
        l_rewards = 0
        ...
        return observation, cost, done, dict(hit=a,
                                            ...
                                            swing_up_success = swing_up_success
                                            )
    ...
    def reset(self):
        ...
        # self.state[2] = np.pi
        self.state[2] = self.np_random.uniform(low=-self.theta_threshold_radians, high=self.theta_threshold_radians)
        self.steps_beyond_done = None

        return np.array(self.state)

    def calibrate_theta(self, theta):
