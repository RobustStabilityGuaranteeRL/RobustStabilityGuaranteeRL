...
class CartPoleEnv_adv(gym.Env):
    ...
    def __init__(self):
        self.cost_weitht = np.array([1., 1., 0., 0., 0.0])
        self.gravity = 9.8066
        # 1 0.1 0.5 original
    ...
    def step(self, action, impulse=0, process_noise=np.zeros([5])):
        ...
        cost = np.sum(self.cost_weitht * np.array([(4/3*x/self.x_threshold)**2,
                                                   (2*theta/ self.theta_threshold_radians)**2,
                                                   abs(x_dot[0]),
                                                   abs(theta_dot[0]),
                                                   (action[0] / self.force_mag) ** 2]))
        # cost_list = []
        # x_base = abs(4/3 *x/self.x_threshold)
        # if x_base<1:
        #     cost_list.append(x_base**2)
        # else:
        #     cost_list.append(x_base ** 4)
        # theta_base = abs(2 * theta / self.theta_threshold_radians)
        # if theta_base < 1:
        #     cost_list.append(theta_base ** 2)
        # else:
        #     cost_list.append(theta_base ** 4)
        # cost_list += [ abs(x_dot[0]), abs(theta_dot[0]), (action[0] / self.force_mag) ** 2]
        # cost = np.sum(self.cost_weitht * np.array(cost_list))
        # if done:
        #     cost = 100.
        l_rewards = 0
